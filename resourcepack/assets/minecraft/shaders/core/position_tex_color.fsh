...
// pass in unnormalized pos and unnormalized gametime
vec4 renderSky(vec3 pos, float gt) {
...
void main() {
    ...
    bool isEndSky = ProjMat[3][3] == 0.0 && all(lessThan(abs(vertexColor.rgb - vec3(40.0/255.0)), vec3(0.01)));
    ...
    // if the end sky does exist, we can draw over it!
    fragColor = renderSky(Pos, GameTime);
